...
@app.cell
def _(pl):
    df = pl.read_csv("longest_flight.csv")
    df.glimpse()
    return (df,)
...
@app.cell
def _(change_altitude, df_5, pl, reach_altitude):
    reach_altitude_time = reach_altitude["time"][0]
    change_altitude_time = change_altitude["time"][0]
    _q = df_5.lazy().filter(
        pl.col("time") < reach_altitude_time, pl.col("current") >= 8
    )
    df_takeoff = _q.collect()
    df_takeoff.glimpse()
    return change_altitude_time, df_takeoff, reach_altitude_time
...
def _(df_5, pl, reach_altitude_time):
    _q = df_5.lazy().filter(
        pl.col("time") < reach_altitude_time, pl.col("altitude") >= 51
    )
    df_climb = _q.collect()
...
@app.cell
def _(df_5, pl):
    last_cruise_speed = df_5.filter(pl.col("speed") > 20).tail()
    return (last_cruise_speed,)


@app.cell
def _(df_5, last_cruise_speed, last_reached_time, pl):
    last_cruise_speed_time = last_cruise_speed["time"][3]
    _q = df_5.lazy().filter(
        pl.col("time") > last_reached_time,
        pl.col("time") < last_cruise_speed_time,
        pl.col("current") > 4,
    )
...
@app.cell
def _(df_5, last_cruise_speed_time, pl):
    _q = df_5.lazy().filter(
        pl.col("time") > last_cruise_speed_time, pl.col("current") > 4
    )
